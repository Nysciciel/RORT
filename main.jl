...
function make_model(inst::Instance)
    ...
    function_capacity = @constraint(model, [i = 1:nb_nodes, f = 1:nb_functions], sum(floww[k] * x[k, i, i, c] * fct_commodities[k, f, c] for k in 1:nb_commodities, c in 1:(nb_functions+1)) <= functions_capacities[f] * y[i, f], base_name = "function_capacity")
